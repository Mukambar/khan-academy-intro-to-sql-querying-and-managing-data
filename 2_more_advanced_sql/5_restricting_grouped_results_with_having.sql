...
SELECT type, SUM(calories) AS total_calories 
FROM exercise_logs
    WHERE calories > 150  /* takes data from column "calories"*/
    GROUP BY type;
...
FROM exercise_logs
    GROUP BY type
    HAVING total_calories > 150; /* takes data from column SUM(calories) or total_calories*/


SELECT type, AVG(calories) AS avg_calories 
FROM exercise_logs
    GROUP BY type
    HAVING avg_calories > 70; /*apply condition to a grouped values*/
